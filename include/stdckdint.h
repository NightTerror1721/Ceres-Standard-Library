...
//
// Stores a op b into *result, wrapped to T the way a two's-complement machine wraps it, and returns true
// when the mathematically exact a op b does not fit T. The operands may be of any integer type and
// different ones - ckd_add(&u, -1, 5) is 4 and fits an unsigned - and T any signed or unsigned integer
// type but char and bool; all of it up to 64 bits, exactly: the operands are carried as a sign and a
// 64-bit magnitude, and a product as wide as 128 bits is still told apart from one that fits.
//
