...
//
// There are PERIPH_PORTS ports. Each is empty, holds a STORAGE medium (512-byte sectors, read and write) or a
// CARTRIDGE (the same, read only). The host plugs files in before the program starts (`ceres run --port 0=stick.img
// --cart 1=game.cart`) or while it runs (a file dropped on the window; the debugger's `attach`). Every connection or
// disconnection is an EVENT: read them with periph_next_event, or wait for one with periph_wait. Media plugged in
// before the program started have their events waiting for it.
//
...
//
// The functions return -1 for a port that does not exist, an empty one, a sector past the end, and a write to a
// cartridge or to a stick the host has write protected. Nothing here needs an interrupt handler; an event also
// raises IRQ_PERIPH (ceres/irq.h) for a program that wants to be woken.
