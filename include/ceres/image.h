...
// Images from files and packs into surfaces (ceres/gfx.h), so sprites and tiles need not be compiled in as arrays.
// Two formats, told apart by their first bytes:
//
...
//   image_free(hero);
//
// A surface is 0x00RRGGBB and has no alpha: a pixel whose alpha is below 128 becomes `transparent` - the colour a
// sprite then keys out - and every other pixel keeps its colour. IMAGE_OPAQUE keeps the colour of every pixel.
//
// Failures return NULL (or -1) with errno: EINVAL for data that is not an image this reads (or is cut short),
// ENOMEM when the pixels do not fit, and whatever fopen said for image_load. The decoders never read outside the
// data they are given. An image is at most IMAGE_MAX_SIDE pixels a side.
