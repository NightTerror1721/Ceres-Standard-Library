...
// Nothing arrives from a plain `ceres run` on a pipe or a file. On a console the keys arrive once the
// program asks for them as they are pressed (term_set_raw(), or key_start() in ceres/key.h, which is what to
// read for a menu or a text field); in the window they always do.
