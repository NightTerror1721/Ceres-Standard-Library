...
// 2D drawing on the pixel display. Everything is drawn into a surface in RAM and shown with
// gfx_present() in one block transfer, so a half-drawn frame is never seen. All of it is integer
// arithmetic (Bresenham lines, midpoint circles, scanline triangles).
//
//   gfx_init(320, 200);
//   for (;;) { gfx_clear(COL_BLACK); gfx_circle_fill(160, 100, 20, COL_RED); gfx_present(); }
//
// Drawing goes to the current TARGET: the screen surface until gfx_set_target() names another. Every
// primitive is clipped to the target's clip rectangle before it touches memory, so drawing partly (or
// wholly) outside is safe and simply loses the part that falls outside.
//
