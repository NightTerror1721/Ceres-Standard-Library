...
// Block devices: what CeresFS (ceres/fs.h) keeps its volumes on. A device moves 512-byte sectors; the machine has
// its internal disk (ceres/disk.h) and the media plugged into the peripheral ports (ceres/periph.h): memory
// sticks, read and write, and cartridges, read only. A program can describe a device of its own - a RAM disk, an
// image in a file - by filling the structure.
