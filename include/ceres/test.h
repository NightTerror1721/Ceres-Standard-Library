...
// output with tests/expected/<name>.expected. Every check that fails prints one FAIL line, and
...
#define CHECK(cond) \
    do { __t_total++; if (!(cond)) { __t_failed++; printf("FAIL %s:%d: %s\n", __FILE__, __LINE__, #cond); } } while (0)

#define CHECK_EQ(a, b) \
    do { __t_total++; if ((a) != (b)) { __t_failed++; \
        printf("FAIL %s:%d: %s == %s (%d vs %d)\n", __FILE__, __LINE__, #a, #b, (int)(a), (int)(b)); } } while (0)
