...
// ranges must not overlap. A range that runs past the end of RAM is clamped: the copy moves what
// fits and dma_transferred() says how much. Arming a transfer clears DONE and sets BUSY; the copy
// lands on the controller's next tick (one instruction later) and raises interrupt 18 (IRQ_DMA),
// which a program may attach a handler to (ceres/irq.h) instead of polling.
//
