...
// A cache of bounded size with least-recently-used eviction: ceres/ds/gmap.h for O(1) lookup by
...
//
// Deviates from a literal "the list node lives inside what gmap stores" design: gmap.h moves every
// slot's bytes on a rebuild (see gmap.c), and an intrusive list_node's neighbors hold its raw
// address - moving it would dangle two pointers. So each entry is its own allocation at a stable
// address, and gmap only ever stores a POINTER to it; the list threads the entries themselves.
