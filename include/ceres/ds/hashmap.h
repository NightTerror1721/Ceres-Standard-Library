...
// A hash map from strings to pointers: open addressing with linear probing, growing before it gets
// crowded. The map keeps its own copy of every key; the values are the caller's pointers and are never
// followed or freed. Removed entries leave a marker so a probe chain is not cut short.
