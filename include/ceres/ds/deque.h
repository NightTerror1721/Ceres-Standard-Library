...
// A double-ended queue of fixed-size elements that grows: push and pop at EITHER end in O(1)
// amortized, and dq_at is random access by logical index. Neither vector.h (only grows from the
// end) nor ringbuf.h (bytes only, fixed capacity, one producer and one consumer) covers this - a
// work queue that is pushed and popped from both ends, or a bounded undo/redo history, wants this
// one.
//
