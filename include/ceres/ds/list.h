...
// A doubly linked list whose node lives INSIDE the object it links: no allocation, and an object can sit
// on several lists at once by carrying several nodes. The list is circular around a sentinel, so no
// operation has an "empty" special case. Header only.
//
