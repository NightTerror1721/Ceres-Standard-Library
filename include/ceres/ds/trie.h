...
// ceres/ds/hashmap.h nor ceres/ds/omap.h answers well - the tree would have to compare whole keys
// at every node, character by character is what a trie is for. Nodes come from a ceres/pool.h the
// caller sizes up front (one node per distinct character position across every key inserted, at
// most - see trie_init), not malloc: the one place among the eighteen collections where a fixed
// capacity is actually the natural shape, since pool.h's own constraint (the block count has to be
// known at init) is no different from what a trie already needs the caller to have thought about.
// A node's children are a ceres/ds/slist.h kept sorted by character, not a 256-entry array - most
// alphabets in practice are sparse at any one branching point.
//
...
//
// The trie never owns what `value` points at - inserting, finding and walking only ever hand that
// pointer back, never follow or free it.
