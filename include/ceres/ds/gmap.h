...
// A hash map from a key of any fixed size to a value of any fixed size - ceres/ds/hashmap.h widened
// past "string to pointer". Open addressing with linear probing, the same shape hashmap.c already
// uses (a tombstone for a removed slot, so a probe chain is never cut short), generalized: a slot's
// "used" state is a byte of its own instead of a magic key pointer, because the key here is bytes
// embedded in the table, not a pointer to something allocated separately. Both the key and the
// value are copied in and out; the map never follows or frees anything the caller gave it a pointer
...
//   gmap_free(&m);
//
// `hash` hashes key_size bytes at the given pointer (or whatever the key actually is, for a key
// like char* whose bytes are not its content); `eq` tells two keys apart the same way pqueue.h's
// comparator does, just for equality instead of order.
