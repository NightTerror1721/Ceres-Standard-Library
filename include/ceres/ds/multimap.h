...
// A map from one key to any number of values - an event's list of listeners, a name's list of
// adjacent nodes - which neither ceres/ds/hashmap.h nor ceres/ds/gmap.h can hold (one key, one
// value each). Each key in the underlying gmap points at the head of a chain of value nodes
// instead of at a value directly; a node is reserved with plain malloc/free rather than
// ceres/pool.h, because pool.h needs its total block count known up front and a chain's length
