...
// A red-black tree whose node lives INSIDE the object it orders: no allocation, no copy, the same
// idea as ceres/ds/list.h taken to a tree - the style Linux's own rbtree.h uses. O(log n) guaranteed
// worst case for insert, remove and find, which neither ceres/ds/iheap.h (a heap only guarantees
// the minimum, not arbitrary find) nor a plain sorted vector (O(n) insert) gives.
//
...
//
// rb_insert does not check for a duplicate key - if the tree may already hold one, rb_find it
// first and decide what a duplicate means for that tree (replace it, keep both, reject the insert).
// The tree does not own what it orders: rb_remove only unlinks a node, it never frees anything.
