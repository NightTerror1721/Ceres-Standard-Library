...
// A Bloom filter: approximate set membership in a fraction of a real set's memory, at the cost of
// occasional false positives (never false negatives) - "have I possibly seen this?" over millions
// of world-generation coordinates or asset keys, where ceres/ds/hset.h would hold every key in full
// but this only ever holds k bits per insertion. There is no bloom_remove: a plain Bloom filter
// cannot forget one item without risking a false negative for another that happens to share a bit.
//
