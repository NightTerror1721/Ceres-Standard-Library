...
// A singly linked list whose node lives INSIDE the object it links: no allocation, one pointer per
// node instead of list.h's two. Good for a stack or a free list, where nothing is ever removed from
// the middle - removing a node other than the head is O(n) here, because there is no `prev` to walk
// back from; that is exactly the operation to reach for list.h over this one for. Header only.
//
