...
// An ordered map of copies, keyed and valued by anything of a fixed size: the convenience layer
// over ceres/ds/rbtree.h, the way ceres/ds/pqueue.h is the convenience layer over vector.h for a
// heap - no need to declare a struct with an rb_node embedded in it just to hold one int key.
//
...
//   omap_free(&m);
//
// Each entry is its own small allocation (key_size + value_size bytes plus an rb_node), unlike
// rbtree.h itself which allocates nothing - this is exactly the trade the header comment above
// promises. A lookup that cannot even allocate its own tiny search probe reports "not found"
// rather than a separate out-of-memory case: nothing later in the call could tell the two apart
// from a bare pointer anyway (see omap.c).
