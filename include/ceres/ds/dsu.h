...
// A disjoint-set (union-find) over the integers 0 .. n-1, on memory the caller provides - the
// textbook structure, no nodes and no allocation of its own, in the same spirit as bitset.h. Answers
// "are these two already in the same group?" in effectively constant time: flood-filling a
// connected region of tiles, Kruskal's minimum spanning tree, grouping entities that have touched.
//
