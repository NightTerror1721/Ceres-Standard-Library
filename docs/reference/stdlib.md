...
General utilities. `long` is 32 bits (`labs`/`ldiv` are the int versions), but `long long` is a real 64-bit type, so `llabs`/`lldiv` and `strtoll`/`strtoull`/`atoll` are 64-bit for real.
