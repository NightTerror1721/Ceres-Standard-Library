...
The syntax: blank lines and lines starting with ; or # are comments; [name] starts a section (keys before the first belong to section ""); key = value (or key: value) with the spaces around both trimmed. A value may be in double quotes, with \" \\ \n \t inside; an unquoted one ends at a ; or # that follows a space. Section and key names compare without regard to case. A key given twice keeps its last value. The text is UTF-8, untouched.

ini_parse() hands each value to a function as it is read, keeping nothing; the struct ini keeps them all, and writes them back grouped by section in the order they first came (comments are not kept).
...
// start or end with a blank; nor may a line come out longer than a reader takes (511 bytes, the value as written):
// they would not read back. Those are EINVAL.
