...
Ceres has one floating-point format: IEEE 754 binary32. `double` and `long double` are `float` (the compiler reduces them, with W2001), so all three families carry the same numbers - unless the program is compiled with -fsoft-double, and then they are a real IEEE binary64 done in software (ceres/f64.h).
