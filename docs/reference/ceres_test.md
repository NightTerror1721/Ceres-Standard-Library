...
A minimal test framework. `ceres run` exits 0 on a clean halt and never with a value the program chose, so a test's verdict is its TEXT: the runner (tools/runtests.ps1) compares the program's output with tests/expected/<name>.expected. Every check that fails prints one FAIL line (file:line and the function it is in), and test_summary() prints either "ALL PASSED n/n" or "FAILED k of n".
