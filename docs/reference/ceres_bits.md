...
The bit instructions, plus helpers built on them. Each one is a macro on the compiler's builtin for its instruction, so it costs that one instruction; the functions in asm/bits.casm remain for whoever takes their address or writes the name in parentheses.
