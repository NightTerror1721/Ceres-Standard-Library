...
A LIFO stack of fixed-size elements - built directly on ceres/ds/vector.h (push/pop/last are already exactly a stack's operations), named separately so that a reader does not have to check that nothing ever calls vector_insert on it to know the order is only ever push/pop.
