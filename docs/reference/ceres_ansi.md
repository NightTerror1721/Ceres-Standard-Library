...
ANSI escape sequences: cursor movement, colours, clearing. The terminal device forwards bytes as they are to the host's standard output, so these work wherever that terminal understands VT sequences (Windows Terminal, VS Code's terminal and most Unix terminals do; the old Windows console needs VT enabled). Colour for a text game before the text grid has per-cell attributes.
