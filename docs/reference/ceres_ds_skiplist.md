...
cmp compares two KEYS - pqueue.h's convention. skl_insert does not check for a duplicate key, the same policy as rb_insert; skl_find only ever returns the first match a search would reach.
