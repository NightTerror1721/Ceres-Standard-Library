...
A set of small integers 0 .. nbits-1 as one bit each, over words the caller provides: occupancy maps, tile flags, "which of these are used". The single-bit operations are inline and do not check the index; bitset_words() says how many words a set of n bits needs.
