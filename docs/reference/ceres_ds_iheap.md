...
A binary heap like ceres/ds/pqueue.h, plus what pqueue.h cannot do: lower an element's priority after it is already inside, or take a specific element out before its turn - both O(log n), which is what Dijkstra and A* need for "this neighbor is now reachable more cheaply" and "abandon this node". pqueue.h cannot do either because once an element is copied in, nothing says where it ended up.

A handle from ih_push stays valid until the element it names is popped or ih_remove'd - it never changes even as the heap itself reorders around it, because a second array (`pos`) tracks where each handle currently sits.
