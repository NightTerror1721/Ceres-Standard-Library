...
Fixed-width integers for a 32-bit machine. `long` is 32 bits, but `long long` is a real 8-byte type (the compiler lowers it as a pair of words), so int64_t/uint64_t are defined here for real.
