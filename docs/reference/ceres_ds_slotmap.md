...
Stable handles into a growable array: removing one entity never invalidates another's handle, and a handle kept too long is detectably stale instead of silently pointing at whatever now lives in that slot. A plain index into a vector.h breaks on the first removal (later indices shift, or a hole has to be skipped by hand); this is what a game with entities dying and being born every frame reaches for instead.
...
Each slot carries a generation, bumped every time it is freed; a handle is `{index, generation}` and is only valid while the two still match. Freed slots are tracked as a small stack of reusable indices rather than a free list threaded through the slot bytes themselves (ceres/pool.h's trick), so this works for any elem_size, including one narrower than a pointer.
