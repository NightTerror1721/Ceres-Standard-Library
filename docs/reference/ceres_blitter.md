...
The 2D blitter (0xFF0C0000, CeresASM 3b2db3b): rectangle operations on RGB32 surfaces in RAM done by the host, at no cost in instructions. A surface is the address of a rectangle's first pixel and its stride (BYTES from one row to the next). ceres/gfx.h uses it on its own when it is there; these are for a program that draws into memory of its own. Every call returns 0, or -1 when there is no blitter or a row ran outside RAM.
