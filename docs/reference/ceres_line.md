...
The host terminal is LINE-ORIENTED: pressing Enter hands the program the whole line at once, several bytes in one go, and the terminal's status only says "there is data", not "there is a character". So these routines take one byte at a time from the terminal and stop at the end of the line, never reading past it: whatever follows the newline stays queued for the next call (or for getchar).

Each call WAITS for a line, or for the end of the input (the host closed stdin: a pipe that ran dry, Ctrl-Z / Ctrl-D at a console); term_eof() says which.
