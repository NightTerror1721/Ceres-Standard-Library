...
(timer_nanos64, timer_ticks64). The struct keeps its two-word shape (the same layout a `uint64_t` has: low word first) so that code written before still builds and links; the timer functions that hand one out are marked deprecated, and nothing in the library uses them any more.
