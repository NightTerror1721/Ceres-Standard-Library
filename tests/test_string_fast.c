// The assembly string routines against the C ones: the same answers for every alignment and length, and
// fewer instructions on a long string (instruction counts are the timer's ticks, the same on every run).
...
int main(void)
{
    ...
    for (int ao = 0; ao < 4; ao++)
        for (int bo = 0; bo < 4; bo++)
        {
            ...
            for (int len = 0; len <= 20; len++)
            {
                make(a, len);
                make(b, len);
                if (strcmp_fast(a, b) != strcmp(a, b)) cmp_ok = 0;              // equal
                for (int at = 0; at < len; at++)
                {
                    make(b, len);
                    b[at] = (char)(b[at] + 3);
                    if (strcmp_fast(a, b) != strcmp(a, b)) cmp_ok = 0;          // differing at every position
                    if (strcmp_fast(b, a) != strcmp(b, a)) cmp_ok = 0;
                }
                make(b, len + 1);
                if (strcmp_fast(a, b) != strcmp(a, b)) cmp_ok = 0;              // one is a prefix of the other
                if (strcmp_fast(b, a) != strcmp(b, a)) cmp_ok = 0;
            }
        }
    CHECK(cmp_ok);
    ...
    CHECK_EQ(strcmp_fast(a, b), strcmp(a, b));
    ...
    for (int off = 0; off < 4; off++)
    {
        ...
        for (int c = 0; c < 256; c++)
        {
            if (strchr_fast(s, c) != strchr(s, c)) find_ok = 0;
            if (memchr_fast(s, c, 30) != memchr(s, c, 30)) find_ok = 0;
            if (memchr_fast(s, c, 31) != memchr(s, c, 31)) find_ok = 0;     // the terminator is inside this one
            if (memchr_fast(s, c, 7) != memchr(s, c, 7)) find_ok = 0;       // and this one ends inside a word
        }
    }
    CHECK(find_ok);
    make(a, 30);
