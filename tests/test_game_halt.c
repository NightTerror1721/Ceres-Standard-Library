...
// (a halted machine's clock jumps to the alarm) instead of spinning on a budget. It
...
int main(void)
{
    ...
    TEST_SECTION("frames pass on the clock, not on a budget");
    CHECK(timer_cpu_hz() >= 1000u);                     // 50 MHz by default
    unsigned int per_ms = timer_cpu_hz() / 1000u;
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        if (waited < 12u * per_ms) long_enough = 0;     // about 20 ms of cycles
